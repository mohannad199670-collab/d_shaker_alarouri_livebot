...
import requests  # 👈 لاستخدام API خارجي للفيديو
...
# ================ إعداد API خارجي للفيديو ================
# مثال: خدمة خارجية تقوم بجلب رابط مباشر للفيديو من يوتيوب
# أنت تضبط هذه القيم من الـ Environment:
# EXT_VIDEO_API_BASE = "https://your-video-api.example.com/get"
# EXT_VIDEO_API_KEY  = "xxxxxxxx" (إن لزم)
EXT_VIDEO_API_BASE = os.getenv("EXT_VIDEO_API_BASE", "").strip()
EXT_VIDEO_API_KEY = os.getenv("EXT_VIDEO_API_KEY", "").strip()
...
def get_available_qualities(video_url: str):
    """
    هنا نستخدم قائمة ثابتة من الجودات الشائعة.
    الـ API الخارجي سيتعامل مع عدم توفر جودة معيّنة ويُسقطها لأقرب جودة.
    """
    return [144, 240, 360, 480, 720, 1080]


def download_video_via_external_api(video_url: str, quality_height: int | None, output_name: str = "source") -> str:
    """
    يستخدم API خارجي لجلب رابط تحميل الفيديو، ثم يقوم البوت بتحميل الملف محلياً.
    - EXT_VIDEO_API_BASE يجب أن يكون مضبوطاً في متغيرات البيئة.
    - نفترض أن الـ API يرجع JSON يحتوي على مفتاح download_url أو url.
    """
    if not EXT_VIDEO_API_BASE:
        raise RuntimeError("❌ لم يتم ضبط EXT_VIDEO_API_BASE في متغيرات البيئة (رابط الـ API الخارجي).")

    params = {
        "url": video_url,
    }
    if quality_height:
        params["quality"] = quality_height

    headers = {}
    if EXT_VIDEO_API_KEY:
        headers["Authorization"] = f"Bearer {EXT_VIDEO_API_KEY}"
    ...
    try:
        ...
        logger.error("Error calling external video API: %s", e)
        raise RuntimeError("فشل الاتصال بالـ API الخارجي للفيديو.")

    if resp.status_code != 200:
        logger.error("External API returned non-200: %s - %s", resp.status_code, resp.text[:200])
        raise RuntimeError("الـ API الخارجي أعاد استجابة غير ناجحة.")

    try:
        data = resp.json()
    except Exception as e:
        logger.error("Error parsing JSON from external API: %s", e)
        raise RuntimeError("فشل解析 استجابة الـ API الخارجي (ليست JSON).")
    ...
    if not download_url:
        logger.error("External API JSON has no download_url/url/file/link key: %s", data)
        raise RuntimeError("الـ API الخارجي لم يُرجع رابط تحميل صحيح.")

    # الآن نحمل الملف من download_url إلى ملف محلي (mp4)
    try:
        r2 = requests.get(download_url, stream=True, timeout=600)
    except Exception as e:
        logger.error("Error downloading video file from external URL: %s", e)
        raise RuntimeError("فشل تحميل الملف من رابط الـ API الخارجي.")

    if r2.status_code != 200:
        logger.error("Download URL returned non-200: %s - %s", r2.status_code, r2.text[:200])
        raise RuntimeError("رابط تحميل الفيديو أعاد استجابة غير ناجحة.")
    ...
    try:
        ...
        logger.error("Error writing downloaded video to file: %s", e)
        raise RuntimeError("فشل حفظ ملف الفيديو على الخادم.")
...
def handle_text(message):
    ...
    if step == "await_url":
        ...
        # الآن فحص الجودات (ثابتة من الدالة)
        bot.reply_to(message, "⏳ يتم تجهيز قائمة الجودات المتاحة…")

        video_url = session["url"]
        try:
            heights = get_available_qualities(video_url)
        except Exception as e:
            logger.error("Error getting qualities list", exc_info=e)
            # لو فشل لأي سبب، نستخدم 360p افتراضياً
            session["quality_height"] = 360
            session["step"] = "choose_mode"
            bot.send_message(
                chat_id,
                "❌ حدث خطأ أثناء تجهيز قائمة الجودات.\n"
        ...
        # إنشاء أزرار حسب الجودات الموجودة
        markup = InlineKeyboardMarkup()
        row = []
...
def handle_callbacks(call):
    ...
    if data.startswith("q_"):
        ...
        if height not in available_heights:
            bot.answer_callback_query(call.id, "⚠️ هذه الجودة غير متاحة في القائمة الحالية.", show_alert=True)
            return
...
def start_cutting(chat_id: int):
    ...
    bot.send_message(
        chat_id,
        "🔧 جاري تحميل الفيديو من الـ API الخارجي وقص المقطع…\n"
    ...
    try:
        # 1) تحميل الفيديو الكامل من API خارجي (بدون كوكيز يوتيوب)
        input_file = download_video_via_external_api(url, quality_height, output_name="source")
        logger.info("Downloaded video file from external API: %s", input_file)
        ...
        # 3) تقسيم المقطع إلى أجزاء (فيديو) بحسب الحد المسموح في تيليجرام
        ...
        bot.send_message(
            chat_id,
            f"❌ حدث خطأ أثناء تحميل أو معالجة الفيديو عبر الـ API الخارجي:\n<code>{e}</code>"
        )
    except Exception as e:
